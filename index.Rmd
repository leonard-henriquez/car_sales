...
plot.forecast <- function(ts, forecast, exp=FALSE) {
  ...
  if(exp==TRUE) {
    df.forecast$upper.1 <- exp(df.forecast$values.pred + qnorm(1-alpha.1/2) * se)
    df.forecast$upper.2 <- exp(df.forecast$values.pred + qnorm(1-alpha.2/2) * se)
    df.forecast$lower.1 <- exp(df.forecast$values.pred - qnorm(1-alpha.1/2) * se)
    df.forecast$lower.2 <- exp(df.forecast$values.pred - qnorm(1-alpha.2/2) * se)
  } else {
...
```{r}
myforecast<-predict(model2,n.ahead=12)
...
```{r}
myforecast<-predict(model,n.ahead=12, exp=TRUE)
plot.forecast(TOTALNSA.ts2, myforecast)
```
